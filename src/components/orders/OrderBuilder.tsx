...
export function OrderBuilder({
  ...
  const recentReports = useMemo(() => savedOrders.slice(0, 3), [savedOrders]);
  ...
  return (
    <section className="panel panel-corners flex flex-col rounded-md p-4">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="panel-title flex items-center gap-2">🛠️ Конструктор заказа</h2>
      ...
      <div className="order-builder-controls mt-4 rounded-md border border-emerald-900/60 bg-black/25 px-3 py-6">
        ...
        <div className="mt-5 flex gap-3">
          <div className="scroll-area panel-inner h-[190px] flex-1 rounded p-1">
            {matrix.length === 0 ? (
              <div className="hud-label col-span-full flex h-full items-center justify-center text-muted">Ничего не найдено</div>
            ) : (
              matrix.map((item) => (
                <button key={`${item.code ?? ""}-${item.name}`} className={`matrix-item ${selected === item.name ? "is-selected" : ""}`} onClick={() => setSelected(item.name)} title={item.en ?? item.name}>
                  <span className="flex min-w-0 items-center gap-3">
                    <GameItemIcon name={item.name} size={28} />
          ...
          <button
            className={`btn h-[190px] w-[58px] flex-col px-0 ${confirmClear ? "btn-danger btn-active" : "btn-danger"}`}
            onClick={handleClear}
      ...
      <div className="scroll-area panel-inner mt-6 max-h-[40vh] min-h-[180px] flex-1 rounded p-2">
        {order.length === 0 ? (
            ...
            {order.map((line) => (
              <li key={line.name} className="row-item group min-w-0">
                <span className="flex min-w-0 items-center gap-2">
                  <GameItemIcon name={line.name} size={32} />
                  <span className="truncate text-[0.82rem] text-white">• {line.name}</span>
                </span>
                <span className="flex shrink-0 items-center gap-1">
                  <button className="btn px-1.5 py-0.5 opacity-60 group-hover:opacity-100" onClick={() => onAdjust(line.name, -1)} title="−1">−</button>
                  <span className="badge-count min-w-[85px]">{line.count} {line.unit}</span>
                  <button className="btn px-1.5 py-0.5 opacity-60 group-hover:opacity-100" onClick={() => onAdjust(line.name, 1)} title="+1">+</button>
                  <button className="btn btn-danger px-1.5 py-0.5 opacity-60 group-hover:opacity-100" onClick={() => onRemove(line.name)} title="Убрать из заказа">✕</button>
                </span>
              </li>
            ))}
          </ul>
      ...
      {/* 📋 Логи заказов — все рапорты штаба, доступны для просмотра/загрузки без необходимости сохранять свой рапорт */}
      <section className="mt-3 rounded border border-white/10 bg-black/25 px-3 py-2.5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <span className="hud-label text-accent">📋 ЛОГИ ЗАКАЗОВ</span>
            <span className="badge-count">{savedOrders.length} ВСЕГО</span>
          </div>
        ...
        <div className="mt-2 flex flex-col gap-1">
          {savedOrdersLoading ? (
            <div className="hud-label py-1 text-muted">Загрузка архива...</div>
          ) : recentReports.length === 0 ? (
            <div className="hud-label py-1 text-muted">Рапортов пока нет — журнал общий для всего штаба.</div>
          ) : (
            recentReports.map((o) => (
              <div key={o.id} className="row-item min-h-[30px] py-0.5">
                ...
                <span className="flex shrink-0 gap-1">
                  <button className="btn px-2 py-0.5" onClick={() => onLoadSaved(o)} title="Загрузить в конструктор">⤴</button>
                  <button className="btn btn-danger px-2 py-0.5" onClick={() => onDeleteSaved(o.id)} title="Удалить рапорт">✕</button>
                </span>
              </div>
            ))
          )}
